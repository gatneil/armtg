...
const resourceTypes = {
	...
	'autogen': (name) => {
	    ...
	    return {"resourceId": resourceTypes['vnet']['fullType'] + '/' + fullVnetSnippet['name'], "resources": [fullVnetSnippet]};
	}
    },
    'pip': {
	...
	Object.keys(newResourcesObj).map((resourceId, index) => {
		...
		if (constraintValueString in newResourcesObj[resourceId]) {
			...
			if (dependencyValue == 'none') {
			    ...
			    var autogeneratedResources = resourceTypes[dependencyCanonicalType].autogen();
			    ...
			    newResourcesObj[resourceId]['templateSnippet']['dependsOn'].push(autogeneratedResources['resourceId']);
			} else {
